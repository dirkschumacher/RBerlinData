## methods for BerlinData generic functions ##

#' @export
getDatasetMetaData.berlin_data_list = function(where, ...) {
  result <- lapply(where, function(x) getDatasetMetaData(x, ...))
  attr(result, "class") <- "berlin_data_list"
  result
}

#' @export
download.berlin_data_list <- function(x, ...) {
  message(paste("Downloading all resources for", length(x), "datasets"))
  y <- lapply(x, function(y) download(y, ...))
  successful.downloads <- Filter(function(r)!is.null(r), y)
  message(paste("Downloaded", length(successful.downloads), "of", length(x), "datasets"))
  successful.downloads
}

## methods for base generic functions ##

#' @export
summary.berlin_data_list <- function(object, ...) {
  cat(paste0(length(object), " dataset", ifelse(length(object)>1, "s", "")))
  for(i in 1:length(object)) {
    cat("\n")
    cat(paste0(i, ": ", object[[i]]$title))
  }
}

#' @export
as.data.frame.berlin_data_list <- function(x, ...) {
  y <- lapply(x, as.data.frame, ...)
  resource_list_lengths <- unlist(sapply(y, nrow))
  y <- do.call(rbind, y)
  y$title <- factor(y$title)
  y$position_in_list <- rep(1:length(resource_list_lengths), times=resource_list_lengths)
  y
}

# roxygen2 doesn't recognize 'is.x' as an S3 method, requires manual documentation
#' @method berlin_data_list is
is.berlin_data_list <- function(x) inherits(x, "berlin_data_list")


# subsets inherit class 
#' @export
`[.berlin_data_list` <- function(x, ...) {
  x <- NextMethod("[")
  class(x) = "berlin_data_list"
  x
}
